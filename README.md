...
- `google_project_service.cloudrun_api` – Enables Cloud Run API
- `random_id.suffix` – Random suffix for unique service name
- `google_cloud_run_v2_service.default` – Cloud Run V2 service
- `google_cloud_run_v2_service_iam_member.noauth` – IAM policy for unauthenticated access
...
3. **Add GitHub secrets**:
    ...
    | `TF_BUCKET_PREFIX` | Bucket prefix/path (e.g., `gcp-cloudrun-container`) |

4. **Run the workflow**:
    - **Apply**: Go to Actions → **CD - GCP Cloud Run Container (Apply)** → fill in all inputs
    - **Destroy**: Go to Actions → **CD - GCP Cloud Run Container (Destroy)** → fill in essential inputs
